...
class CandidateURLPolicy:
    ...
    LISTING_PATHS = ("/search", "/results", "/browse", "/authors", "/author/", "/category/", "/tag/")
    LISTING_QUERY_KEYS = {"searchtype", "query", "q", "page", "offset", "start"}
    BINARY_PATH_MARKERS = ("/pdf/", "/pdf", ".pdf", ".docx", ".xlsx", ".pptx", "ptpmcrender.fcgi")
    ...
    def rejection_reason(cls, url: str) -> URLRejectionReason | None:
        ...
        if any(path == marker or path.startswith(marker + "/") for marker in cls.AUTH_PATHS):
            return URLRejectionReason.AUTHENTICATION_URL
        if any(marker in path or marker in parsed.query.lower() for marker in cls.BINARY_PATH_MARKERS):
            return URLRejectionReason.BINARY_DOCUMENT_URL
        if any(path == marker or path.startswith(marker + "/") for marker in cls.LISTING_PATHS):
            return URLRejectionReason.SEARCH_LISTING_URL
        if parse_qs(parsed.query) and any(key.lower() in cls.LISTING_QUERY_KEYS for key in parse_qs(parsed.query)):
            return URLRejectionReason.SEARCH_LISTING_URL
        if path == "/":
