...
class AdaptiveConfig(BaseModel):
    ...
    browser_navigation_timeout_seconds: float = Field(
        default=5.0, ge=2.0, le=120.0, description="Bounded browser navigation timeout"
    )
...
class RateLimitConfig(BaseModel):
    ...
    default_host_rps: float = Field(
        default=5.0, description="Default requests per second per host"
    )
    max_host_concurrency: int = Field(
        default=8, description="Maximum concurrent requests per host"
    )
...
class SecurityConfig(BaseModel):
    max_response_size_bytes: int = Field(
        default=100 * 1024 * 1024, description="Maximum response size (100MB)"
    )
    max_decompressed_size_bytes: int = Field(
        default=500 * 1024 * 1024, description="Maximum decompressed size (500MB)"
    )
...
class BudgetConfig(BaseModel):
    max_pages: int = Field(
        default=50000, description="Maximum pages to process per job"
    )
    max_depth: int = Field(default=10, description="Maximum crawl depth")
    max_bytes: int = Field(
        default=10 * 1024 * 1024 * 1024, description="Maximum network bytes (10GB)"
    )
    browser_seconds: int = Field(
        default=3600, description="Maximum browser execution time in seconds"
    )
