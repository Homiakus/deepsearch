...
from scraper.evidence.extractor import evidence_extractor
...
class DeepSearchPipeline:
    ...
    async def execute(self, opts: DeepSearchPipelineOptions) -> DeepSearchPipelineResult:
        ...
        session_evidence_store = EvidenceStore()
        ...
        while frontier.size() > 0 and len(acquired_results) < opts.max_pages:
            ...
            current_url = item.candidate.url
            c_url = item.candidate.canonical_url or canonicalize_url(current_url)
            ...
            try:
                ...
                if item.depth < opts.depth and len(acquired_results) < opts.max_pages:
                    ...
                    for dlink in discovered_links:
                        if not dlink.is_navigation and not dlink.is_footer:
