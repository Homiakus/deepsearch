...
class AuthorizedBrowserManager:
    ...
    def __init__(self, user_data_dir: Optional[str] = None):
        self.user_data_dir = os.path.abspath(user_data_dir or getattr(settings, "browser_user_data_dir", ".browser_profile"))
        os.makedirs(self.user_data_dir, exist_ok=True)

    async def launch_interactive_session(self, target_url: str = "https://annas-archive.cc", timeout_sec: int = 120) -> str:
        ...
        logger.info(f"Starting Interactive Authorized Browser session (user profile: {self.user_data_dir})...")
        ...
        async with async_playwright() as p:
            ...
            print(f"\n[INFO] Opening browser window at: {target_url}")
            print(f"[INFO] Complete any required log in, captcha, or file download in the browser window.")
            print(f"[INFO] Closing window will automatically save your session cookies to: {state_path}\n")
    ...
    async def download_file(
        ...
        if not PLAYWRIGHT_AVAILABLE:
            return DownloadResult(success=False, url=url, error_message="Playwright is not installed.")
        ...
        async with async_playwright() as p:
            try:
                ...
                logger.info(f"Successfully downloaded file: {suggested_filename} ({file_size} bytes)")
                await context.close()
