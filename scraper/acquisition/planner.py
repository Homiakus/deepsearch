...
from scraper.acquisition.models import AcquisitionRequest, AcquisitionResult, QualityReport
...
class DomainTelemetry:
    ...
    def record(self, domain: str, backend: str, success: bool, quality: float, latency_ms: float):
        ...
        if key not in self.stats:
            ...
            if success:
                entry["successes"] += 1.0
            entry["quality_ewma"] = (1 - self.alpha) * entry["quality_ewma"] + self.alpha * quality
            entry["latency_ewma"] = (1 - self.alpha) * entry["latency_ewma"] + self.alpha * latency_ms
...
class BackendPlanner:
    ...
    def should_escalate(
        ...
        if result.quality.score >= 0.7 and not result.quality.blocked and not result.quality.likely_unrendered:
            return False, None

        suggested = result.quality.suggested_escalation
        candidates = [
            b for b in available_backends
            if b.name != current_backend.name and b.base_cost >= current_backend.base_cost
        ]
