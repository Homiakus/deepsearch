...
# -------------------------------------------------------------------
async def fetch_arxiv_pdfs(
    queries: List[str], max_per_query: int = 4
) -> List[Dict[str, Any]]:
...
async def download_pdf(client: httpx.AsyncClient, pdf_url: str, filename: str) -> str:
    ...
    try:
        res = await client.get(
            pdf_url, headers=HEADERS, follow_redirects=True, timeout=25.0
        )
        if res.status_code == 200 and (
            len(res.content) > 2000
            or res.headers.get("content-type", "").lower().count("pdf")
        ):
            with open(dest_path, "wb") as f:
                f.write(res.content)
            logger.info(
                f"Successfully downloaded PDF: {filename} ({len(res.content)} bytes)"
            )
            return dest_path
        else:
...
async def main():
    ...
    for item in downloaded_pdf_files:
        ...
        try:
            ...
            chunks = [
                full_pdf_text[i : i + 1500] for i in range(0, len(full_pdf_text), 1200)
            ]
            for c_idx, chunk in enumerate(chunks):
                ...
                with open(rag_file, "w", encoding="utf-8") as rf:
                    rf.write(
                        f"Source: {pdf_title} (Page Chunk {c_idx + 1})\nURL: {item['source_url']}\n\n{chunk}"
                    )
