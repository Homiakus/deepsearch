...
def mock_acquisition_engine():
    ...
    async def mock_acquire(url, canonical_url, mode=ExecutionMode.BALANCED, cached_content=None, take_screenshot=False):
        ...
        html_content = f"<html><body><h1>Title for {url}</h1><p>Sample content for research topic.</p><a href='{url}/subpage'>Sublink</a></body></html>"
...
async def test_archive_exporter_files_and_rag_structure(mock_acquisition_engine):
    ...
    # Create fake image file on disk for copy test
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # 1. Verify `files/` folder contents (for user with links)
        files_folder = os.path.join(dir_out, "files")
        user_files = os.listdir(files_folder)
        assert len(user_files) == 1

        with open(os.path.join(files_folder, user_files[0]), "r", encoding="utf-8") as f:
            user_content = f.read()
            assert "Source Link" in user_content
            assert "https://physics.org/quantum" in user_content

        # 2. Verify `media/` folder contents
        media_folder = os.path.join(dir_out, "media")
        assert os.path.exists(media_folder)
        archived_media = os.listdir(media_folder)
        assert len(archived_media) == 1
        assert archived_media[0] == "img_01_quantum_diagram.png"

        # 3. Verify `manifest.json` entries
        with open(os.path.join(dir_out, "manifest.json"), "r", encoding="utf-8") as mf:
            m_data = json.load(mf)
            assert m_data["summary"]["total_media_files"] == 1
            inventory_types = [item.get("type") for item in m_data["inventory"]]
            assert "image" in inventory_types

        # 4. Verify `rag/` folder contents (for LLM)
        ...
        with open(os.path.join(rag_folder, "rag_context.md"), "r", encoding="utf-8") as f:
            ctx_text = f.read()
            assert "Topic Visual Media Gallery" in ctx_text

        # 5. Test ZIP packing
        zip_path = os.path.join(tmp_dir, "test_out.zip")
        exporter.pack_zip_archive(dir_out, zip_path)
