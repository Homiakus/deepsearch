...
def test_self_healing_selector_fallback():
    ...
    original_html = (
        '<div class="product"><span class="price-tag">Price: $100</span></div>'
    )
    ...
    modified_html = (
        '<div class="product"><span class="amount-label">Price: $100</span></div>'
    )

    # Primary selector breaks, but self-healing fingerprint matches the node
    matched_node = SelfHealingSelector.match_element(
        modified_html, "span.price-tag", fingerprint=fp
    )
