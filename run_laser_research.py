...
async def run_research():
    ...
    for url in TARGET_URLS:
        ...
        try:
            ...
            extraction = ExtractionEngine.extract_from_html(artifact.url, artifact.text_content)
            
            acquired_results.append((artifact, extraction))
            logger.info(f"Successfully processed {url} (Markdown len: {len(extraction.clean_markdown)})")
    ...
    logger.info(f"Archive generated at {zip_path} with {len(acquired_results)} processed sources.")
